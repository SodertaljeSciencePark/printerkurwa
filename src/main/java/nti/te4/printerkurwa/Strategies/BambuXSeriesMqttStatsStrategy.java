...
public class BambuXSeriesMqttStatsStrategy implements StatsStrategy {
                        ...
                        client.subscribeWith()
                                ...
                                .whenComplete((subAck, subThrowable) -> {
                                    if (subThrowable != null) {
                                        ...
                                        log.info("Linked! Listening on X-Series stats from: {} on topic: {}", printer.getIp(), topic);
                                    }
                                });
                    });
    ...
    public void stopListening(UUID printerId) {
        ...
        if (client != null) {
            log.info("Stopping X-Series MQTT listener for printer ID: {}", printerId);
            client.disconnect();
        }
    }
    ...
    private void handleMessage(Mqtt3Publish publish, Printer printer, Map<UUID, PrinterStats> statsMap) {
        try {
            ...
            log.error("Error occurred while handling MQTT message for X-Series printer: {}", printer.getName());
        }
    }
}
